...
df_dataset <- read_tsv(
    ...
    col_types = cols(
      ...
      gage_comid = col_character(),
      huc_InsufficientObs_7Day = col_character(),
      huc_InsufficientObs_30Day = col_character(),
      gage_InsufficientObs_7Day = col_character(),
      gage_InsufficientObs_30Day = col_character()
    ),
    na = "NA"
  ) %>% 
  mutate(id = site_no) %>%
...
out_dataset <- df_dataset %>% 
  select(id, lat = gage_lat, lon = gage_long, variables$df$id)
...
layer_sf <- df_dataset %>% 
  select(id, site_no, huc12 = huc12_fpp, dec_lat_va = gage_lat, dec_long_va = gage_long) %>% 
